...
public class MyPageViewHandler {
    ...
    public void whenStared_then_UPDATE_1(@Payload StarUpdated starUpdated) {
        try {
            if (starUpdated.isMe()) {
                ...
                myPageRepository.save(myPage);
                System.out.println("##### listener whenStared_then_UPDATE_1 [VIEW] : " + starUpdated.toJson());            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenStared_then_UPDATE_1(@Payload StarCanceled starCanceled) {
        try {
            if (starCanceled.isMe()) {
                // view 객체 조회
                List<MyPage> myPageList = myPageRepository.findByResrvNo(starCanceled.getResrvNo());
                MyPage myPage = new MyPage();
                myPage.setResrvNo(starCanceled.getResrvNo());
                if (myPageList.size() > 0) {
                    myPage = myPageList.get(0);
                }
                myPage.setStars(starCanceled.getStars());
                myPageRepository.save(myPage);
                System.out.println("##### listener whenStared_then_UPDATE_1 [VIEW] : " + starCanceled.toJson());            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
